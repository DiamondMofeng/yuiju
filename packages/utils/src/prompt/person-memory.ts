...
- 如果旧对象不存在，只有当这次互动已经足以支撑建立人物页时，shouldUpdate 才能为 true。
...
- 如果 shouldUpdate=false，reviewPassed=true，且不要调用 "reviewPersonMemoryProposal"。
- 如果 shouldUpdate=true，只有在最近一次 "reviewPersonMemoryProposal" 返回 approved=true 后，reviewPassed 才能为 true。
- 如果 shouldUpdate=true 且你在 3 次审查内仍无法得到通过结果，则 reviewPassed=false。
`.trim();
}
