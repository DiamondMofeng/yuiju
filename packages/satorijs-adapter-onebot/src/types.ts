import { camelize, type Dict, Logger } from "@satorijs/core";
import type { CQCode } from "./bot";
...
export interface Internal {
  sendPrivateMsg(
    user_id: id,
    message: string | readonly CQCode[],
    auto_escape?: boolean,
  ): Promise<number>;
  sendPrivateMsgAsync(
    user_id: id,
...
// biome-ignore lint/suspicious/noUnsafeDeclarationMerging: OneBot methods are generated from action names at module load time.
export class Internal {
  ...
  private async _get<T = any>(action: string, params = {}): Promise<T> {
    ...
    if (!response) throw new Error(`OneBot request is not ready: ${action}`);
  ...
  static define(name: string, ...params: string[]) {
    ...
    (Internal.prototype as any)[prop] = async function (this: Internal, ...args: any[]) {
    ...
    if (isAsync) {
      (Internal.prototype as any)[prop + "Async"] = async function (
        this: Internal,
        ...args: any[]
      ) {
        await this._get(
