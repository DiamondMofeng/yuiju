import { h, hyphenate, Logger, Universal } from "@satorijs/core";
import * as qface from "qface";
import { type BaseBot, CQCode } from "./bot";
import type * as OneBot from "./types";
...
export const decodeGuildMember = (user: OneBot.SenderInfo): Universal.GuildMember => ({
  ...
  name: user.card,
  roles: user.role ? [{ id: user.role, name: user.role }] : [],
});
...
export const adaptQQGuildMemberInfo = (user: OneBot.GuildMemberInfo): Universal.GuildMember => ({
  ...
  name: user.nickname,
  roles: user.role_name ? [{ id: user.role_name, name: user.role_name }] : [],
});

export const adaptQQGuildMemberProfile = (
  user: OneBot.GuildMemberProfile,
): Universal.GuildMember => ({
  user: {
    id: user.tiny_id,
    name: user.nickname,
  ...
  name: user.nickname,
  roles: user.roles?.map((r) => ({ id: r.role_name, name: r.role_name })) || [],
});
  ...
  if (bot.config.advanced?.splitMixedContent) {
    chain.forEach((item, index) => {
      ...
      if (
        right &&
        right.type === "text" &&
        right.attrs.content.trimStart() === right.attrs.content
      ) {
...
const decodeGuildChannelId = (data: OneBot.Message): [string | undefined, string | undefined] => {
  if (data.guild_id) {
...
export async function adaptSession(bot: BaseBot, data: OneBot.Payload) {
  ...
  if (data.target_id) (session as any).targetId = "" + data.target_id;
  ...
  if (data.post_type === "request") {
    ...
    switch (data.notice_type) {
      ...
      case "offline_file":
        ...
        session.event.subtype = "offline-file-added";
        break;
      case "group_upload":
        session.elements = [h("file", data.file)];
        session.type = "message";
        session.subtype = "group";
        session.event.subtype = "guild-file-added";
        break;
      default:
        return;
    }
